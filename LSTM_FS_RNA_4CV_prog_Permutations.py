...
#Runs through all permutations of the 5 folds for training order (120) to ensure that order does not affect model performance

#Code also contains other methods to use Denseblock model structure
...
#Function to output data from LSTM model - called during each iteration/permuataion
...
def main():
    ...
    #Steps to implement
    #1) Load the data
    #location of RNA data in correct format for each patient
    ...
    #feature selection can be enabled or disabled to use all features
    ...
    outtodf = ['perm', 'holdout_index', 'loss_tr', 'mae_tr', 'loss_val', 'mae_val']
    ...
    #testing cycle list/fixed list for chosen network size
    db_list = [32]
    lstm_lst = [32]
    dense_units = [32]
        ...
        for lst in lstm_lst:
            for dus in dense_units:
                ...
                for train_index, test_index in kf.split(X):
                    folds.append((X[train_index], y[train_index], X[test_index], y[test_index]))
                ...
                for perm in permutations_of_folds:
                    ...
                    loss, mae = model.evaluate(X_holdout, y_holdout, verbose=0)
                    print(f"Holdout Fold: {holdout_index}, Loss: {loss}, MAE: {mae}")
                    results.append((perm, holdout_index, loss_tr, mae_tr, loss, mae))
                    perm_ct +=1
                    
                    out_data = [perm, holdout_index, loss_tr, mae_tr, loss, mae]
